...
void get_fractiles_per_accs(int *membarr, unsigned char *fracarr, int period);  
void date_from_sec(char *preddate, time_t secs);
time_t sec_from_date(char *stim);

int main(int argc, char *argv[])
{
  int32_t X,Y,A,A1,A0,R1,R2,dR,i,m,*membarr_orig,arrsize,N,a,PASS,M;
  ...
  accpref=getenv("RAVAKE_ACCPREF");
  if(!accpref) accpref=def_accpref;
  ...
  /* Geometry initialization */
  {
     ...
     hdrfile=getenv("RAVAKE_ODIM_HDF5_HDRFILE");
     if(!hdrfile) exit(1);
  ...
  if(EPSG) OSRImportFromEPSG(hSRS,EPSG); else
  {
    char projstr[200];
 
    sprintf(projstr,"%s +ellps=sphere +units=m +no_defs",projdef); 
    OSRImportFromProj4(hSRS,projstr);
  }
  ...
  { /* parse accumulation thresholds and read the data */
    ...
    while(1)
    {
      ...
      while(1)
      {
        ...
        acclims[thri][period]=(int32_t)(10000.00001*f_acclims[thri][period]);
        if(!pc) break;
        thrline=pc+1;
        thri++;       
      }
    ...
    for(i=0;i<2;i++)
    {
     ...
     for(i=0;i<timesteps;i++)
     {
       ...
       if(min5flags[i]) /* the same data could be used in several datasets, so read only once */
       {
         ...
         sprintf(accname,"%s/%s_%s-%s+%03d.dat",accdir,accpref,obstime,fortime,formins);
         printf("%s\n",accname);
         RAVACCF=fopen(accname,"r");
         ...
         for(m=0;m<members;m++)
         {
           ...
           fread(&ravacc[i][m][0],membsize,1,RAVACCF);
	   /*	   	   
           sprintf(moname,"%s/MONITOR_%s-%s-%d.pgm",accdir,obstime,fortime,m);
           MONF=fopen(moname,"w");
           fprintf(MONF,"P5\n%ld %ld\n255\n",xsize,ysize);
	   for(k=0;k<arrsize;k++) monacc[k]=ravacc[i][m][k]/100;
  ...
  for(p=0;p<Periods;p++)
  {
    ...
    printf("Generating %s\n",HDFoutfile);
    sprintf(source,"ORG:EFKL,PLC=%s_area",Area);
    ...
    H5LTset_attribute_string(OUTH5,"how","nodes","NOD:fikor,NOD:fivan,NOD:fianj,NOD:fiika,NOD:fikes,NOD:fipet,NOD:fikuo,NOD:fivim,NOD:fiuta,NOD:filuo");
	...
	*/
        for(N=0;N<arrsize;N++)
	  {
		...
		memset(membarr,0,submems*sizeof(int));
		for(m=1;m<members;m++)
		{
		    R2=membarr_orig[m];
		    R1=membarr_orig[m-1];
			...
			M=(m-1)*10+i;
			if(!R2) membarr[M]=0;
			else membarr[M]=R1-dR*i;
		    }
		}
		membarr[submems-1]=membarr_orig[members-1];

		memset(fracarr,100,Thresholds[p]); /* initialize probabilities to 100% */
...
void get_fractiles_per_accs(int32_t *membarr, unsigned char *fracarr, int period)  
{
  ...
  thi=Thresholds[period]-1;
  for(m=1;m<submems;m++)
  {
    if(thi<0) break;
    Rmin=acclims[period][thi+1];
    R2=membarr[m];
    R1=membarr[m-1];
    /*    if(!R1) break; */
    if(Rmin>R1)
